...
class ChipsController extends AppController {
    ...
    public function registra_reg_chips($idExcel = null) {
        ...
        if ($excel['Excel']['tipo'] == 'asignacion') {
            ...
            $objPHPExcel = $objLector->load("../webroot/files/$nombre_ex");
            $i = 0;
            while ($excel['Excel']['total_registros'] > 0 && $excel['Excel']['total_registros'] > $excel['Excel']['puntero'] && $i < 70) {
                $row = $objPHPExcel->getActiveSheet()->getRowIterator($puntero)->current();
                $cellIterator = $row->getCellIterator();
                $cellIterator->setIterateOnlyExistingCells(false);
                foreach ($cellIterator as $cell) {
                    ...
                    if ('A' == $cell->getColumn()) {
                        ...
                        if (count($fechaExcel) > 1) {
                            $array_data[$cell->getColumn()] = substr($fechaExcel[2], 0, 4) . '-' . $fechaExcel[0] . '-' . $fechaExcel[1];
                        } else {
                ...
                if (!empty($array_data['C'])) {
                    $verifica_tel = $this->Activado->find('first', array('conditions' => array('Activado.phone_number' => $array_data['C'], 'Activado.fecha_act' => $array_data['A'])));
                    if (empty($verifica_tel)) {
                        //$da_acti['ciudad_nro_tel'] = $array_data['A'];
                        $da_acti['fecha_act'] = $array_data['A']; //A
                        $da_acti['fecha_doc'] = $array_data['B']; //B
                        //$da_acti['plan_code'] = $array_data['D'];
                        //$da_acti['description'] = $array_data['E'];
                        $da_acti['phone_number'] = $array_data['C']; //C
                        $da_acti['dealer_code'] = $array_data['D']; //D
                        //$da_acti['dealer'] = $array_data['H'];
                        //$da_acti['dealer_nom_act'] = $array_data['I'];
                        $da_acti['subdealer_code'] = $array_data['E']; //E
                        $da_acti['subdealer'] = $array_data['F']; //F
                        $da_acti['subdealer_nom_act'] = $array_data['G']; //G
                        //$da_acti['canal_m'] = $array_data['M'];
                        $da_acti['canal_n'] = $array_data['H']; //mercado H
                        $da_acti['inspector'] = $array_data['J']; //I
                        $da_acti['comercial'] = $array_data['J']; //J
                        $da_acti['excel_id'] = $idExcel;
                        $this->Activado->create();
                        $this->Activado->save($da_acti);
                        $chip = $this->Chip->find('first', array(
                            'recursive' => -1,
                            'conditions' => array('Chip.telefono' => $array_data['C'], 'Chip.activado_id' => NULL),
                            'fields' => array('Chip.id'),
                            'order' => array('Chip.fecha DESC')
                        ));
                        if (!empty($chip)) {
                            $d_chip['activado_id'] = $this->Activado->getLastInsertID();
                            $this->Chip->id = $chip['Chip']['id'];
    ...
    public function eliminar_ac($idExcel = null) {
        ...
        /* $chips = $this->Chip->find('all',array(
          'recursive' => 0,
          'conditions' => array('Activado.excel_id' => $idExcel),
          'fields' => array('Chip.id','Activado.id'),
          'limit' => 20
          )); */

        $sql1 = "(SELECT Activado.id, chips.id FROM activados AS Activado LEFT JOIN chips ON (chips.activado_id = Activado.id) WHERE Activado.excel_id = $idExcel LIMIT 50)";

        $chips = $this->Activado->query($sql1);
        ...
        $array = array();
        foreach ($chips as $ch) {
            if (!empty($ch['chips']['id'])) {
                $d_chip['activado_id'] = NULL;
                $this->Chip->id = $ch['chips']['id'];
                $this->Chip->save($d_chip);
            }
            $this->Activado->delete($ch['Activado']['id']);
        }

        $array['numero'] = $this->Activado->find('count', array(
            'recursive' => -1,
            'conditions' => array('Activado.excel_id' => $idExcel)
        ));

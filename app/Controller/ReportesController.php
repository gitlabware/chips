...
//debug($hoy);exit;
    ...
    public function reporte_chips() {
        ...
        if (!empty($this->request->data['Dato'])) {
            ...
            $this->Chip->virtualFields = array(
                'distribuidor' => "CONCAT(Persona.nombre,' ',Persona.ap_paterno,' ',Persona.ap_materno)",
                'activados' => "COUNT(Chip.activado_id)"
            );
            $datos = $this->Chip->find('all', array(
                ...
                'joins' => array(
                    array(
                        ...
                        'type' => 'INNER',
                        'conditions' => array(
                            'Persona.id = Distribuidor.persona_id'
                        )
                    )),
    ...
    public function reporte_chips_clientes() {
        ...
        if (!empty($this->request->data['Dato'])) {
            ...
            if ($activado_t == 1) {
                $condiciones['Chip.activado_id !='] = NULL;
            } else {
                $condiciones['Chip.activado_id'] = NULL;
            }
